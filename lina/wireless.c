...
//     printf("%s\n",args[i-1]);
      ...
      pid = fork ( ) ;
      if ( pid == 0 ) { /* child process */ close ( pipe2 [ 0 ] ) ;
          close ( pipe2 [ 1 ] ) ;
//     signal(SIGUSR1,sigproc);
          if ( fork ( ) != 0 ) exit ( 0 ) ; /* to avoid zombie */ close ( pip [ 0 ] ) ;
          close ( pip [ 0 ] ) ;
          close ( 2 ) ;
//     dup(1);
          close ( 1 ) ;
...
//         exit(0);
  ...
  int CheckSystemd ( void ) {
      ...
      if ( CheckProcess ( "systemd" ) ) {
          ...
          if ( EnvFile [ 0 ] == '\0' ) runjob ( "cat /usr/lib/systemd/system/wpa_supplicant.service" , ProcessEnv ) ;
