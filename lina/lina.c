...
//     if(fork()!=0) exit(0); /* to avoid zombie */
      if ( pid == 0 ) { /* child process */ close ( 0 ) ;
          dup ( pip2 [ 0 ] ) ;
          close ( pip2 [ 0 ] ) ;
          close ( 1 ) ;
          dup ( pip [ 1 ] ) ;
...
//   pipew =pip2[1];
      ...
      cid = fork ( ) ;
//     if(fork()!=0) exit(0); /* to avoid zombie */
      if ( cid == 0 ) { /* child process */ close ( 0 ) ;
          dup ( pip2 [ 0 ] ) ;
          close ( pip2 [ 0 ] ) ;
          close ( 1 ) ;
          open ( "/dev/null" , O_WRONLY | O_CREAT , 0777 ) ;
...
//     kill(pid,SIGUSR1);
      ...
      else { /* parent process */ close ( pip2 [ 0 ] ) ;
          close ( pip [ 1 ] ) ;
          Sid = cid;
...
//  strcpy(xpm0, (char *)"##/root/lina/WRK/thara.png");
      xpm0 = imgs [ 0 ] ;
      DIP p0 = {
      'p' , 349 , 198 , 765 , 354 , ( void * ) xpm0 , 12 , /* bkgr colour */ 0 , 0 /* border hide */ , 0.2 };
      ...
      e1 [ 0 ] .img = NULL;
      DIT t1 = {
      't' , 415 , 268 , 645 , 303 , 24 , 1 , 1 , e1 , 1 , 1 , NULL , linatextbox1callback , 0 , 0 , 18 , 9 /* args , Call back */ };
...
//    450,240,  
//    500,290,  
      'p' , 430 , 220 , 480 , 270 , ( void * ) xpm3 , -1 , /* bkgr colour */ 0 , 0 /* border hide */ , 0.0 };
...
//  strcpy(xpm0,"##login.png");
      ...
      'h' , 645 , 263 , 675 , 313 , 2 , 4 , 28 , 42 , 1 , 1 , 0 , 0.500000 , 0 , 0 , 0 , 1 , /* button type and roundinfg factor 
          ( 0-0.5 ) , bordr , hide , nodrawbkgr*/ butn4 , linasplbutton1callback , /* Callbak */ NULL /* any args */ };
...
//    488,236,  
//    718,278,
      'n' , 480 , 231 , 700 , 266 , 2 , 2 , 200 , 24 , 1 , 1 , 10 , 0.2500000 , 0 , 0 , 0 , 1 , /* button type and roundinfg factor 
          ( 0-0.5 ) , bordr , hide , nodrawbkgr*/ butn5 , linabutton1callback , /* Callbak */ NULL /* any args */ };
...
//  butn6[2].xpmn= (void *)&wireless1_str;
      ...
      'h' , 1142 , 590 , 1460 , 636 , 30 , 3 , 44 , 44 , 4 , 1 , 0 , 0.500000 , 0 , 0 , 0 , 1 , /* button type and roundinfg factor 
          ( 0-0.5 ) , bordr , hide , nodrawbkgr*/ butn6 , linasplbutton2callback , /* Callbak */ NULL /* any args */ };
          
      strcpy ( h6.Wid , ( char * ) "BottomLine" ) ;
      char *xpm7;
      xpm7 = ( char * ) DateTimeImg ( & lc , 300 , 100 ) ;
      DIP p7 = {
      'p' , 1000 , 20 , 1300 , 120 , ( void * ) xpm7 , 12 , /* bkgr colour */ 0 , 0 /* border hide */ , 0.3 };
...
*************************************************/
      ...
      v [ 1 ] = NULL;
      v [ 0 ] = ( void * ) ( v0 ) ;
      void *pt = NULL; /* pointer to send any extra information */ GrpId = linaGroup 
          ( D , v , pt ) ;
...
//  ModifylinaGc(&(D.gc));    /*  set colors for gui*/
      d = D.d;
      D.bkup = 1; /* set to 1 for backup */ D.bor_type = 0;
      D.df = 6;
      D.tw = 4;
      D.bw = 4;
      D.lw = 4;
      D.rw = 4;
      D.xo = 10; /* Position of Dialog */ D.yo = 10;
      D.xl = 1307; /* Length of Dialog */ D.yl = 624;
           /* Width of Dialog */ D.Initfun = linainit;
           /* init fuction for Dialog */ D.Cleanupfun = linacleanup; /* init fuction for Dialog */ D.kbattn = 0;
           /* 1 for drawing keyborad attention */ D.butattn = 0; /* 1 for drawing button attention */ D.fullscreen = 1;
           /* 1 for for fullscreen mode */ D.Deco = 0; /* 1 for Window Decorration */ D.transparency = 1.0;
           /* float 1.0 for full transparency */ D.Newwin = 0; /* 1 for new window not yet implemented */ D.NoWinMngr = 1;
//  D.Bkpixmap = "##/etc/xdg/lina/background.jpg";    /*  background image */
      D.DrawBkgr = 1; /* 1 for drawing background */ D.Bkpixmap = Img;
#if 1 
#else 
      D.Sticky = 0; /* 1 for stickyness */ D.Resize = 0;
           /* 1 for Resize option */ D.MinWidth = 100;
           /* for Resize option */ D.MinHeight = 100; /* for Resize option */ D.Callback = linaCallBack;
           /* default callback */ D.Callback = NULL;
#endif
//  strcpy(D.name,"Kulina Designer ver 1.0");    /*  Dialog name you may change */
      D.ResizeCallback = linaResizeCallBack; /* Resize callback */ if 
          ( lc.ShowTime == 0 ) D.WaitCallback = NULL;
           /* Wait callback */ else D.WaitCallback = linaWaitCallBack; /* Wait callback */ D.Fixpos = 1;
           /* 1 for Fixing Position */ D.NoTaskBar = 1; /* 1 for not showing in task bar*/ D.StackPos = 0;
      ...
      // D.xo=D.yo=0; D.xl = xres-10; D.yl=yres-80;
          ...
          t-> y2 = t-> y1 + yl;
//     lc.KbVis=0;
          kgDefineColor ( 501 , ( unsigned char ) lc.ButRed , ( unsigned char ) lc.ButGreen , 
              ( unsigned char ) lc.ButBlue ) ;
          kgDefineColor ( 201 , ( unsigned char ) lc.Red , ( unsigned char ) lc.Green , 
...
//     D.StackPos = 1; // you may need it
      } /* end of fullscreen mode */ printf ( "Calling Ui\n" ) ;
      ret = kgUi ( & D ) ;
      kgFreeImage ( Img ) ;
      Img = NULL;
      kgCleanUi ( & D ) ;
...
*************************************************/
      ...
      void* v [ 1 ] ;
      v [ 0 ] = ( void * ) ( v0 ) ;
      void *pt = NULL; /* pointer to send any extra information */ lina 
          ( NULL , v , pt ) ;
      return NULL;
  }
#else
  int CheckDisplay ( void ) {
      ...
      if ( Img == NULL ) {
          ...
          if ( Img == NULL ) {
              ...
              RGBtoHSV ( ( float ) lc.Red , ( float ) lc.Green , 
                  ( float ) lc.Blue , & H , & S , & V ) ;
              V *= 0.4;
              HSVtoRGB ( & R , & G , & B , H , S , V ) ;
              kgChangeColor ( fid , 10 , ( int ) R , ( int ) G , ( int ) B ) ;
              kgRoundedRectangleFill ( fid , Xres*0.5 , Yres*0.5 , 
                  ( Xres+2.0 ) , Yres+2.0 , 0 , 10 , 0. ) ;
              Img = kgGetResizedImage ( fid ) ;
              kgCloseImage ( fid ) ;
          }
  ...
  int logindia ( int ( *ProcessOut ) ( int , int , int ) ) {
      ...
      pid = fork ( ) ;
//     if(fork()!=0) exit(0); /* to avoid zombie */
      if ( pid == 0 ) { /* child process */ printf ( "Calling CheckDisplay\n" ) ;
          fflush ( stdout ) ;
          CheckDisplay ( ) ;
          close ( 0 ) ;
          dup ( pip2 [ 0 ] ) ;
...
//  setuid(uid);
  ...
  void RunSession_n ( int Index , char *session ) {
      char buff [ 200 ] ;
      sprintf ( buff , "su -l -s /bin/bash -c %s %s" , session , GetUserId 
          ( & lc , Index ) ) ;
      signal ( SIGHUP , HangupSignal ) ;
#if 0
      setsid ( ) ;
  ...
  int CheckXserver ( void ) {
      ...
      while ( fgets ( buff , 299 , pp ) != NULL ) {
          sscanf ( buff , "%d%s%s%s" , & Id , tty , dummy , program ) ;
          if ( ( strcmp ( "X" , program ) == 0 ) || 
              ( strcmp ( "Xorg" , program ) == 0 ) ) {
              Okay = Id;
              break;
          }
      }
...
*************************************************/
      ...
      void *pt = NULL; /* pointer to send any extra information */ FILE *pp;
#ifndef D_STOPSERVER
      if ( ! isdisplayinuse ( 0 ) ) {
#if 0
      }
