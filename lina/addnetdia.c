...
//  char *xpm0=   (char *)MakePixmapImage( 378,125,lc.Red,lc.Green,lc.Blue,0.05);
      ...
      DIP p0 = {
//    383,130,  
      'p' , 2 , 2 , 520 , 332 , ( void * ) xpm0 , -1 , /* bkgr colour */ 3 , 0 , 0.1 /* border hide */ };
      ...
      e1 [ 1 ] .noecho = 1;
      DIT t1 = {
      't' , 13 , 16 , 372 , 80 , 20 , 1 , 2 , e1 , 1 , 1 , NULL , addnetdiatextbox1callback , 0 , 0 , 18 , 9 /* args , Call back */ };
      ...
      butn2 [ 0 ] .butncode = '';
      DIL h2 = {
      'h' , 157 , 90 , 239 , 123 , 2 , 0 , 72 , 25 , 1 , 1 , 4 , 0.500000 , 0 , 0 , 0 , 1 , /* button type and roundinfg factor 
          ( 0-0.5 ) , bordr , hide , nodrawbkgr*/ butn2 , addnetdiasplbutton1callback , /* Callbak */ NULL /* any args */ };
...
*************************************************/
      ...
      v [ 0 ] = ( void * ) ( v0 ) ;
      v [ 1 ] = ( void * ) ( v1 ) ;
      void *pt = NULL; /* pointer to send any extra information */ GrpId = addnetdiaGroup 
          ( D , v , pt ) ;
  ...
  int addnetdia ( void *parent , void **v , void *pt ) {
      ...
      D.lw = 4;
      D.rw = 4;
      D.xo = 475; /* Position of Dialog */ D.yo = 174;
      D.xl = 522; /* Length of Dialog */ D.yl = 334; /* Width of Dialog */ D.Initfun = addnetdiainit;
           /* init fuction for Dialog */ D.Cleanupfun = addnetdiacleanup; /* init fuction for Dialog */ D.kbattn = 0;
           /* 1 for drawing keyborad attention */ D.butattn = 0; /* 1 for drawing button attention */ D.fullscreen = 0;
           /* 1 for for fullscreen mode */ D.Deco = 1; /* 1 for Window Decorration */ D.transparency = 1.0;
          
#if 1 
#else 
      D.Newwin = 0; /* 1 for new window not yet implemented */ D.DrawBkgr = 0;
           ...
           /* for Resize option */ D.MinHeight = 100; /* for Resize option */ D.Callback = addnetdiaCallBack;
...
//  strcpy(D.name,"Kulina Designer ver 1.0");    /*  Dialog name you may change */
      D.ResizeCallback = addnetdiaResizeCallBack; /* Resize callback */ D.WaitCallback = NULL;
           /* Wait callback */ D.WaitCallback = addnetdiaWaitCallBack; /* Wait callback */ D.Fixpos = 1;
...
*************************************************/
      ...
      v0 [ 0 ] = '\0';
      v1 [ 0 ] = '\0';
      void *pt = NULL; /* pointer to send any extra information */ addnetdia 
          ( arg , v , pt ) ;
      i = 0; while ( v0 [ i ] == ' ' ) i++;
      if ( v0 [ i ] > ' ' ) {
          ret = 1;
