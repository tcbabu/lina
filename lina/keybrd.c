...
static void *MakePixmapImage(int xl,int yl,int r,int g,int b,float fac) {
   ...
   kgChangeColor(fid,553,(int)rf,(int)gf,(int)bf);
   kgRoundedRectangleFill(fid,(float)xl*0.5,(float)yl*0.5,(float)xl+1,(float)yl+1,0,553,fac);
   kgRoundedRectangleFill(fid,(float)xl*0.5,(float)yl*0.5,(float)xl,(float)yl,0,552,fac);
   kgRoundedRectangleFill(fid,(float)xl*0.5,(float)yl*0.5,(float)xl-4,(float)yl-4,0,551,fac);
...
int keybrdGroup( DIALOG *D,void **v,void *pt) {
  ...
  DIN b1 = { 
    ...
    24, 
    13,2, 
    9,0.000000,0,0,0,1, /* button type and roundinfg factor(0-0.5),bordr,hide ,nodrawbkgr*/
